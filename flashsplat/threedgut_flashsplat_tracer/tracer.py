...
def load_3dgut_plugin(conf):
    ...
    if _3dgut_plugin is None:
        try:
            from . import lib3dgut_flashsplat_cc as tdgut  # type: ignore
        except ImportError:
            from .setup_3dgut_flashsplat import setup_3dgut_flashsplat

            tdgut = setup_3dgut_flashsplat(conf)
        _3dgut_plugin = tdgut
...
class Tracer:
    ...
    @torch.no_grad()
    def accumulate_contribution(self, gaussians, gpu_batch: Batch, object_id_mask, contribution, frame_id=0):
        """FlashSplat pass: render one view and accumulate each Gaussian's alpha-blending
        weight into ``contribution[object_id]``.

        Args:
            object_id_mask: int32 CUDA tensor, ``[H*W]`` or ``[H, W]``, object id per pixel
                (0 = background, negative = ignore this pixel).
            contribution: float32 CUDA tensor ``[num_objects, num_gaussians]``, **modified
                in place**. Allocate once and pass it across every view of the sweep.

        Returns the same dict as :meth:`render`, so the caller can also save the RGB.
        """
        rays_o = gpu_batch.rays_ori
        rays_d = gpu_batch.rays_dir
        ...
        particle_radiance = gaussians.get_features().contiguous()

        ray_time = (
            torch.ones(
                (rays_o.shape[0], rays_o.shape[1], rays_o.shape[2], 1), device=rays_o.device, dtype=torch.long
            )
            * poses.timestamps_us[0]
        )

        pred_rgba, pred_dist, hits_count, mog_visibility = self.tracer_wrapper.trace_contrib(
            frame_id,
            gaussians.n_active_features,
            particle_density,
            particle_radiance,
            ...
            object_id_mask.reshape(-1).to(torch.int32).contiguous(),
            contribution,
        )
        ...
        pred_rgb, pred_opacity = gaussians.background(
            gpu_batch.T_to_world.contiguous(), rays_d, pred_rgb, pred_opacity, False
        )
