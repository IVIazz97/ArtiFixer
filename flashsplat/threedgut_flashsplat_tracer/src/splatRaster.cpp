...
SplatRaster::traceImpl(uint32_t frameNumber, int numActiveFeatures,
                       ...
                       const int32_t* rayObjectIdCudaPtr,
                       float* particleContributionCudaPtr) {
...
SplatRaster::trace(uint32_t frameNumber, int numActiveFeatures,
    ...
    return traceImpl(frameNumber, numActiveFeatures, particleDensity, particleRadiance,
                     rayOrigin, rayDirection, rayTimestamp, sensorModel,
                     startTimestamp, endTimestamp, sensorsStartPose, sensorsEndPose,
                     /*rayObjectIdCudaPtr=*/nullptr, /*particleContributionCudaPtr=*/nullptr);
}

std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
SplatRaster::traceContrib(uint32_t frameNumber, int numActiveFeatures,
                          torch::Tensor particleDensity,
                          torch::Tensor particleRadiance,
                          torch::Tensor rayOrigin,
                          torch::Tensor rayDirection,
                          ...
                          torch::Tensor sensorsStartPose,
                          torch::Tensor sensorsEndPose,
                          torch::Tensor rayObjectId,
                          torch::Tensor particleContribution) {

    const int64_t numPixels     = rayOrigin.size(1) * rayOrigin.size(2);
    const int64_t numParticles  = particleDensity.size(0);

    TORCH_CHECK(rayObjectId.scalar_type() == torch::kInt32,
                "[3dgut][flashsplat] rayObjectId must be int32, got ", rayObjectId.scalar_type());
    TORCH_CHECK(rayObjectId.is_cuda() && rayObjectId.is_contiguous(),
                "[3dgut][flashsplat] rayObjectId must be a contiguous CUDA tensor");
    TORCH_CHECK(rayObjectId.numel() == numPixels,
                "[3dgut][flashsplat] rayObjectId has ", rayObjectId.numel(),
                " elements, expected ", numPixels, " (H*W)");

    TORCH_CHECK(particleContribution.scalar_type() == torch::kFloat32,
                "[3dgut][flashsplat] particleContribution must be float32, got ",
                particleContribution.scalar_type());
    TORCH_CHECK(particleContribution.is_cuda() && particleContribution.is_contiguous(),
                "[3dgut][flashsplat] particleContribution must be a contiguous CUDA tensor");
    TORCH_CHECK(particleContribution.dim() == 2 && particleContribution.size(1) == numParticles,
                "[3dgut][flashsplat] particleContribution must be [numObjects, ", numParticles,
                "], got a tensor with ", particleContribution.dim(), " dims and last dim ",
                particleContribution.dim() == 2 ? particleContribution.size(1) : -1);

    return traceImpl(frameNumber, numActiveFeatures, particleDensity, particleRadiance,
                     rayOrigin, rayDirection, rayTimestamp, sensorModel,
                     startTimestamp, endTimestamp, sensorsStartPose, sensorsEndPose,
                     rayObjectId.data_ptr<int32_t>(),
                     particleContribution.data_ptr<float>());
}
