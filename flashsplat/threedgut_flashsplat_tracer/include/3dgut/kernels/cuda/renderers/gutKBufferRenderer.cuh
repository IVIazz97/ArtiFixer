...
struct GUTKBufferRenderer : Params {
    ...
    static inline __device__ void processHitParticle(
        ...
        const threedgut::FlashSplatAccumulator& flashSplat = {}) {
...
#pragma unroll
            ...
            // FlashSplat: this particle's contribution to the label of the pixel it is
            // being blended into. Summed over every pixel of every view, this is the
            // linear coefficient the FlashSplat solver assigns labels from.
    ...
    static inline __device__ void eval(const threedgut::RenderParameters& params,
                                       ...
                                       threedgut::FlashSplatAccumulator flashSplat                        = {}) {
        ...
        if constexpr (Backward && (Params::KHitBufferSize == 0)) {
            ...
            evalKBuffer(ray, particles, tileParticleRangeIndices, tileNumBlocksToProcess, tileNumParticlesToProcess, tileThreadIdx,
                        sortedTileParticleIdxPtr, particleFeaturesBuffer, particleFeaturesGradientBuffer, flashSplat);
        }
    }
    ...
    static inline __device__ void evalForwardNoKBufferBalanced(
        ...
        for (uint32_t j = laneId; j < alignedParticleCount; j += WarpSize) {
            ...
            if (shouldContribute) {
                ...
                // FlashSplat: same accumulation as processHitParticle, per warp lane.
                if (flashSplat.contribution != nullptr) {
                    const int32_t objId = flashSplat.rayObjectId[ray.idx];
                    if (objId >= 0) {
                        atomicAdd(&flashSplat.contribution[objId * flashSplat.numParticles + particleIdx],
                                  hitWeight);
                    }
                }
            }
