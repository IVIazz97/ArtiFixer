...
private:
    ...
    /// Shared forward implementation behind `trace` and `traceContrib`.
    /// The two FlashSplat pointers are null for a plain render.
    std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
    traceImpl(uint32_t frameNumber, int numActiveFeatures,
...
public:
    ...
    /// FlashSplat segmentation pass: same forward render as `trace`, but additionally
    /// accumulates each particle's alpha-blending weight (alpha * T) into
    /// `particleContribution[objectId]`, where `objectId` is read per pixel from
    /// `rayObjectId`. `particleContribution` is caller-owned and accumulated in place
    /// across views, so a multi-view sweep allocates it exactly once.
    std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
    traceContrib(uint32_t frameNumber, int numActiveFeatures,
                 // Particles
                 torch::Tensor particleDensity,
                 torch::Tensor particleRadiance,
                 // Rays
