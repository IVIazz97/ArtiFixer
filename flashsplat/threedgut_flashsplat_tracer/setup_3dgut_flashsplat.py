...
# The vendored copy keeps its own sources but reuses the 3DGRUT-ArtiFixer submodule for
# tiny-cuda-nn and the slang include path, so there is only one copy of those on disk.
THREEDGRUT_ROOT = os.path.join(
    os.path.dirname(__file__), "..", "..", "thirdparty", "3DGRUT-ArtiFixer"
)


# ----------------------------------------------------------------------------
#
def setup_3dgut_flashsplat(conf):

    build_dir = torch.utils.cpp_extension._get_build_directory("lib3dgut_flashsplat_cc", verbose=True)

    include_paths = []
    prefix = os.path.dirname(__file__)
    tcnn_root = os.path.join(THREEDGRUT_ROOT, "thirdparty", "tiny-cuda-nn")
    include_paths.append(os.path.join(prefix, "include"))
    include_paths.append(os.path.join(tcnn_root, "include"))
    include_paths.append(os.path.join(tcnn_root, "dependencies"))
    include_paths.append(os.path.join(tcnn_root, "dependencies", "fmt", "include"))
    ...
    jit.compile_slang_kernel(
        ...
        include_paths=[
            ...
            os.path.join(THREEDGRUT_ROOT, "threedgrt_tracer", "include"),
        ],
        defines=defines,
    )
